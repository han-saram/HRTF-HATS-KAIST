...
% for ele = R_ele
    ...
    for azim_idx = 1:N_azim
        ...
        % magnitude
        HRTF_L(azim_idx,:) = 20*log10(abs(H_L(1:N/2)));
        HRTF_R(azim_idx,:) = 20*log10(abs(H_R(1:N/2)));
             

        % spectral cues
        P_HRTF_L = islocalmax(HRTF_L(azim_idx,:));
        N_HRTF_L = islocalmin(HRTF_L(azim_idx,:));
        P_HRTF_R = islocalmax(HRTF_R(azim_idx,:));
        N_HRTF_R = islocalmin(HRTF_R(azim_idx,:));

        temp = find(P_HRTF_L == 1);
        P_HRTF_L(temp(find(freq_axis(P_HRTF_L) < 3))) = 0;
        temp = find(N_HRTF_L == 1);
        N_HRTF_L(temp(find(freq_axis(N_HRTF_L) < 3))) = 0;
        temp = find(P_HRTF_R == 1);
        P_HRTF_R(temp(find(freq_axis(P_HRTF_R) < 3))) = 0;
        temp = find(N_HRTF_R == 1);
        N_HRTF_R(temp(find(freq_axis(N_HRTF_R) < 3))) = 0;
        
        temp = freq_axis(P_HRTF_L);
        if length(temp) == 2
            spec_cues(azim_idx,1:2) = temp(1:2);
            spec_cues(azim_idx,3) = temp(2);
        elseif length(temp) == 1
            spec_cues(azim_idx,1:3) = temp(1);
        else
            spec_cues(azim_idx,1:3) = temp(1:3);            
        end
        temp = freq_axis(N_HRTF_L);
        if length(temp) == 2
            spec_cues(azim_idx,4:5) = temp(1:2);
            spec_cues(azim_idx,6) = temp(2);
        elseif length(temp) == 1
            spec_cues(azim_idx,4:6) = temp(1);
        else
            spec_cues(azim_idx,4:6) = temp(1:3);
        end
        temp = freq_axis(P_HRTF_R);
        if length(temp) == 2
            spec_cues(azim_idx,7:8) = temp(1:2);
            spec_cues(azim_idx,9) = temp(2);
        elseif length(temp) == 1
            spec_cues(azim_idx,7:9) = temp(1);
        else
            spec_cues(azim_idx,7:9) = temp(1:3);
        end
        temp = freq_axis(N_HRTF_R);
        if length(temp) == 2
            spec_cues(azim_idx,10:11) = temp(1:2);
            spec_cues(azim_idx,12) = temp(2);
        elseif length(temp) == 1
            spec_cues(azim_idx,10:12) = temp(1);
        else
            spec_cues(azim_idx,10:12) = temp(1:3);
        end
    end
    ...
    N3_R = spec_cues(:,12);
    Azim = azims;
    T = table(Azim,P1_L,P2_L,P3_L,N1_L,N2_L,N3_L,P1_R,P2_R,P3_R,N1_R,N2_R,N3_R);
    eval(['writetable(T,''..\00_Data\PN_HATS\HRTF_cues_el\e' num2str(ele) '.txt'',''Delimiter'',''\t'',''WriteRowNames'',true);'])
    
    Ang = reshape([azims azims azims],[],1);
    ...
    figure
    im = imagesc(freq_axis,azims,HRTF_L,m_lims); axis xy
    im.AlphaData = .8;
    colormap jet
    hold on
    ...
    set(gca,'FontName','times','FontSize',18);
    title(['Spectral cues of left ear at \phi=',num2str(ele),'\circ'],'FontSize',18)
    xlabel('Frequency, kHz','FontSize',18)
    ylabel('\theta, deg','FontSize',18)
