...
%   Synopsis : plot IR & TF at ref. point measured by Mic-R in 1 direction
%	Algorithm : -
%-------------------------------------------------------------------------
...
% mic directions (-40, -30, 0, +30, +60, +90 deg)
mics = [-40, -30, 0, 30, 60, 90];
N_mic = length(mics);
...
%% elevation X deg, mic dir. X deg
% elevation
elev = 45;                      % -40 ~ +90 deg

% mic direction
mic_dir = 30;                   % -40, -30, 0, +30, +60, +90 deg

% root of directory
root = '../00_Data/TF_Ref';

% load IR
[h,Fs] = ir_ref1_F(elev,mic_dir,root);
N = length(h);
...
title(['IR amplitude at Ref. point ( Elevation ',num2str(elev),' deg, Mic dir. ',num2str(mic_dir),' deg )'],'fontsize',14);
...
title(['TF magnitude at Ref. point ( Elevation ',num2str(elev),' deg, Mic dir. ',num2str(mic_dir),' deg )'],'fontsize',14);
...
title(['TF phase at Ref. point ( Elevation ',num2str(elev),' deg, Mic dir. ',num2str(mic_dir),' deg )'],'fontsize',14);
axis([0 freq_axis(end) p_lims(1) p_lims(2)]);
grid on
set(gcf,'position',[300 300 800 1000]);

%% elevation -40 ~ +90 deg, mic dir. -40, -30, 0, +30, +60, +90 deg
for mic_idx = 1:N_mic
    % mic direction
    mic_dir = mics(mic_idx);
    ...
    title(['IR amplitude at Ref. point ( Mic dir. ',num2str(mic_dir),' deg )'],'fontsize',14);
    ...
    title(['TF magnitude at Ref. point ( Mic dir. ',num2str(mic_dir),' deg )'],'fontsize',14);
    ...
    colormap jet
    xlabel('Frequency [kHz]','fontsize',12); ylabel('Elevation [deg.]','fontsize',12);
    title(['TF phase at Ref. point ( Mic dir. ',num2str(mic_dir),' deg )'],'fontsize',14);
    axis([0 freq_axis(end) elevs(1) elevs(end)]);
    grid on
